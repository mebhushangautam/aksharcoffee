...
<div class="min-w-0 max-w-full overflow-x-hidden">
    @if ($tableQrOrderingBlocked ?? false)
        <div class="px-4 mt-4">
            <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100" role="alert">
                @lang('messages.tableQrDineInDisabled')
            </div>
        </div>
    @endif
    ...
    @if(!$isHeaderDisabled)
        <section class="min-w-0 max-w-full overflow-x-hidden px-4 bg-white dark:bg-gray-900">
            @if($headerType === 'text')
            ...
            @if($headerType === 'image' && count($headerImages) > 0)
                <div
                    wire:key="cart-header-carousel-{{ count($headerImages) }}"
                    class="relative w-full min-w-0 max-w-full touch-pan-y overflow-hidden"
                    x-data="{
                        active: 0,
                        total: {{ count($headerImages) }},
                        autoplay: null,
                        startAutoplay() {
                ...
                >
                    <div class="relative w-full aspect-[1248/192] overflow-hidden border border-gray-200 rounded-lg shadow-lg dark:border-gray-700">
                        ...
                        <div class="absolute inset-x-0 bottom-3 z-30 flex justify-center gap-2 sm:bottom-5 sm:gap-3">
                            @foreach($headerImages as $index => $image)
                                <button type="button"
                                        class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full transition-all duration-200"
                                        :class="active === {{ $index }} ? 'bg-white dark:bg-gray-200' : 'bg-white/50 dark:bg-gray-200/50 hover:bg-white/75 dark:hover:bg-gray-200/75'"
                        ...
                        <button type="button" class="absolute top-0 z-30 items-center justify-center hidden h-full px-2 cursor-pointer start-0 sm:flex sm:px-4 group focus:outline-none" @click="prev()">
                        ...
                        <button type="button" class="absolute top-0 z-30 items-center justify-center hidden h-full px-2 cursor-pointer end-0 sm:flex sm:px-4 group focus:outline-none" @click="next()">
                            <span class="inline-flex items-center justify-center w-8 h-8 transition-all duration-200 rounded-full sm:w-10 sm:h-10 bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                                <svg class="w-3 h-3 text-white sm:w-4 sm:h-4 dark:text-gray-200 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
            ...
            @if($headerType !== 'text' && !($headerType === 'image' && count($headerImages) > 0))
                <div class="py-4 px-4 mx-auto max-w-screen-xl text-center lg:py-8 lg:px-12 bg-skin-base/[.1] dark:bg-gray-800 rounded-lg">
                    <h1 class="text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-3xl dark:text-white">
    ...
    @if (!$showCart)
        ...
        @if (!$cameFromTableQR || !$tableQrOrderingBlocked)
        ...
        <div class="flex min-w-0 max-w-full flex-col overflow-x-hidden px-4 my-4" x-data="{ showAll: false }">
            <!-- Card Section -->
        ...
        <div class="mx-4 mt-6 min-w-0 max-w-full overflow-x-hidden">
            <!-- Mobile Dropdown -->
            <div class="relative min-w-0 lg:hidden" x-data="{ open: false }">
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
                    ...
                    class="absolute inset-x-0 z-50 mt-2 overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-700">
    ...
    @if ($showMenu && !$showOrderTypeModal && !$useClientMenuCatalog)
        <div class="min-w-0 max-w-full overflow-x-hidden px-4 mb-32 space-y-4 lg:gap-8"
            x-data="{
            ...
            @forelse ($this->menuItems as $key => $itemCat)
                ...
                <div class="space-y-4 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-8">
                    @foreach ($itemCat as $item)
                        <div @class([
                            'flex min-w-0 max-w-full items-center justify-between gap-4 border shadow-sm rounded-lg hover:shadow-md transition dark:border-gray-600 dark:lg:bg-gray-900 dark:shadow-sm lg:rounded-md',
                            'bg-gray-100 dark:bg-gray-800' => !$item->in_stock,
                            'bg-white dark:bg-gray-900' => $item->in_stock,
                                ]) wire:key='menu-item-{{ $item->id . microtime() }}'>
                             <div class="flex min-w-0 w-full gap-3 p-3 sm:gap-4">
                                ...
                                <div
                                    ...
                                    <div
                                        class="flex flex-col gap-1.5 text-sm font-semibold text-gray-900 lg:text-base dark:text-white">
                                        <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                                            <span class="inline-flex min-w-0 flex-1 cursor-pointer items-center"
                                                wire:click="showItemDetail({{ $item->id }})">
                                                <img src="{{ asset('img/' . $item->type . '.svg') }}" class="h-4 shrink-0 me-1"
                                                    title="@lang('modules.menu.' . $item->type)" alt="" />
                                                <span class="min-w-0 break-words">{{ $item->getTranslatedValue('item_name', session('locale')) }}</span>
                                            </span>
                                        </div>
                                            ...
                                            @if (count($menuCardAllergenKeys) > 0)
                                                <div class="flex w-full min-w-0 flex-wrap gap-1.5" wire:click.stop>
                                                    @foreach ($menuCardAllergenKeys as $menuAllergenKey)
                                                        @php
                                                            $menuAllergenLabel = __(\App\Support\EuAnnexIiAllergens::langKey($menuAllergenKey));
                                                        @endphp
                                                        <span class="inline-flex max-w-full items-center gap-1.5 rounded-md border border-amber-200/85 bg-amber-50/95 px-2 py-1 dark:border-amber-700/50 dark:bg-amber-950/35">
                                                            <img src="{{ \App\Support\EuAnnexIiAllergens::defaultIconUrl($menuAllergenKey) }}"
                                                                alt=""
                                                                class="h-4 w-4 shrink-0 object-contain"
                                                                width="16"
                                                                height="16"
                                                                loading="lazy" />
                                                            <span class="max-w-[12rem] truncate text-xs font-medium leading-tight text-gray-800 dark:text-gray-100 sm:max-w-[14rem]">{{ $menuAllergenLabel }}</span>
                                                        </span>
                                                    @endforeach
                                                </div>
                                            @endif
                                        ...
                                        @if (count($menuCardDietaryKeys) > 0)
                                            <div class="flex w-full min-w-0 flex-wrap gap-1.5" wire:click.stop
                                                ...
                                                @foreach ($menuCardDietaryKeys as $menuDietaryKey)
                                                    @php
                                                        $menuDietaryLabel = __(\App\Support\DietaryLabels::langKey($menuDietaryKey));
                                                    @endphp
                                                    <span class="inline-flex max-w-full items-center gap-1.5 rounded-md border border-emerald-200/85 bg-emerald-50/95 px-2 py-1 dark:border-emerald-700/50 dark:bg-emerald-950/35">
                                                        <img src="{{ \App\Support\DietaryLabels::defaultIconUrl($menuDietaryKey) }}"
                                                            alt=""
                                                            class="h-4 w-4 shrink-0 object-contain"
                                                            width="16"
                                                            height="16"
                                                            loading="lazy" />
                                                        <span class="max-w-[12rem] truncate text-xs font-medium leading-tight text-emerald-900 dark:text-emerald-100 sm:max-w-[14rem]">{{ $menuDietaryLabel }}</span>
                                                    </span>
                                                @endforeach
                                            </div>
                                        @endif
                                    ...
                                    <div class="flex items-center justify-between w-full">
                                        ...
                                        @if ($canCreateOrder && !($tableQrOrderingBlocked ?? false))
                                            @if (!$item->in_stock)
            ...
            @include('livewire.shop.partials.mobile-shop-actions')

            @if ($cartQty > 0)
                <div
                    class="fixed inset-x-0 bottom-1 z-10 px-4 lg:px-0">
                    <div class="mx-auto flex max-w-lg min-w-0 items-center justify-between gap-2 p-4 antialiased font-bold text-white rounded-md bg-skin-base dark:bg-gray-800 lg:max-w-screen-xl">
                        <div class="min-w-0 flex-1 break-words text-sm leading-snug sm:text-base">@lang('modules.order.totalItem'): {{ $cartQty }} &nbsp;|&nbsp;
                            {{ currency_format($subTotal, $restaurant->currency_id) }} + @lang('modules.order.taxes')</div>

                        <x-secondary-button class="shrink-0" wire:click="showCartItems">@lang('modules.order.viewCart')</x-secondary-button>
                    </div>
                </div>
            @endif
        </div>
    ...
    @if (!$showCart && $useClientMenuCatalog)
        ...
        @if ($this->shopCartStripShouldDisplay())
            <div class="fixed inset-x-0 bottom-1 z-10 px-4 lg:px-0">
                <div class="mx-auto flex max-w-lg min-w-0 items-center justify-between gap-2 p-4 antialiased font-bold text-white rounded-md bg-skin-base dark:bg-gray-800 lg:max-w-screen-xl">
                    <div class="min-w-0 flex-1 break-words text-sm leading-snug sm:text-base">@lang('modules.order.totalItem'): {{ $this->shopCartBannerLineCount() }} &nbsp;|&nbsp;
                        {{ currency_format($subTotal, $restaurant->currency_id) }} + @lang('modules.order.taxes')</div>
    ...
    @if ($showCart)
        ...
        @endphp
        <div class="min-w-0 max-w-full overflow-x-hidden px-4 mt-4 space-y-4">
            @foreach ($orderItemList as $key => $item)
                <div class="flex min-w-0 max-w-full items-center justify-between gap-4 transition bg-white border rounded-lg shadow-sm hover:shadow-md dark:border-gray-600 dark:lg:bg-gray-900 dark:shadow-sm"
                    wire:key='menu-item-{{ $item->id . microtime() }}'>
                    <div class="flex min-w-0 w-full gap-3 p-4 dark:bg-gray-800 dark:text-gray-200 sm:gap-4">
                        <!-- Item Image -->
                        @if ($restaurant && !$restaurant->hide_menu_item_image_on_customer_site)
                        ...
                        <div class="flex-1 min-w-0">
                            <div
                                ...
                                <div class="flex w-full min-w-0 flex-col gap-1.5">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <div
                                            ...
                                            <img src="{{ asset('img/' . $item->type . '.svg') }}" class="h-4 shrink-0 me-2"
                                                title="@lang('modules.menu.' . $item->type)" alt="" />
                                            {{ $item->item_name }}
                                        </div>
                                ...
                                <div class="flex min-w-0 max-w-full flex-wrap items-center justify-between gap-3 sm:w-auto md:max-w-[33%] md:flex-shrink-0">
                                    <!-- Quantity Controls -->
                                    ...
                                    @endphp
                                    <div class="flex min-w-0 max-w-full flex-col items-end gap-1 text-end">
                                        <div class="max-w-full break-words text-xs text-gray-500 dark:text-gray-400 sm:whitespace-nowrap">
                                        ...
                                        <span
                                            class="max-w-full break-words text-base font-semibold text-gray-900 dark:text-white sm:whitespace-nowrap">
                                            ...
                                            <div class="flex w-full">
                                                <div class="relative flex-1">
                                                    <x-input x-ref="noteInput" type="text"
                                                        ...
                                                        class="w-full pe-20 text-sm border border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-skin-base focus:border-skin-base"
                                                        :placeholder="__('placeholders.addItemNotesPlaceholder')"
                                                        @keydown.enter="showNoteInput = false" />
                                                    <div
                                                        class="absolute inset-y-0 end-0 flex items-center gap-1 pe-2">
                                                        <button
