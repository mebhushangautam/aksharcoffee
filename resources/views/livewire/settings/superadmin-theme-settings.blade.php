<div
    ...
    <form wire:submit="submitForm" x-data="{ photoName: null, photoPreview: null, darkPhotoPreview: null }">
        <div class="space-y-6">
            {{-- Logo Upload Section --}}
            ...
            <div class="p-6 bg-gray-900 rounded-lg shadow-sm border border-gray-700">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h4 class="text-base font-medium text-white">{{ __('modules.settings.darkModeLogo') }}</h4>
                        <p class="mt-1 text-sm text-gray-400">
                            {{ __('modules.settings.darkModeLogoDescription') }}</p>
                    </div>
                ...
                <div class="flex items-center space-x-6">
                    <div class="flex-shrink-0">
                        <div class="relative h-24 w-24">
                            <div x-show="!darkPhotoPreview"
                                class="h-24 w-24 rounded-lg bg-gray-800 flex items-center justify-center overflow-hidden border border-gray-700">
                                <img src="{{ $settings->dark_logo_url }}" alt="{{ $settings->name }}"
                                    class="h-24 w-24 object-contain">
                            </div>

                            <div x-show="darkPhotoPreview" style="display: none;">
                                <span class="block h-24 w-24 rounded-lg bg-gray-800 border border-gray-700 bg-cover bg-center bg-no-repeat"
                                    x-bind:style="'background-image: url(\'' + darkPhotoPreview + '\');'">
                                </span>
                            </div>

                            <div wire:loading wire:target="darkPhoto"
                                class="absolute inset-0 bg-gray-900/60 rounded-lg flex items-center justify-center">
                                <svg class="animate-spin h-6 w-6 text-white" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10"
                    ...
                    <div class="flex-grow space-y-3">
                        <input type="file" id="darkPhoto" class="hidden"
                            accept="image/png, image/gif, image/jpeg, image/webp, image/svg+xml" wire:model.defer="darkPhoto"
                            x-ref="darkPhoto"
                            x-on:change="
                                const reader = new FileReader();
                                reader.onload = (e) => { darkPhotoPreview = e.target.result; };
                                reader.readAsDataURL($refs.darkPhoto.files[0]);
                            " />

                        <div class="flex flex-wrap gap-3">
                            <x-secondary-button type="button" x-on:click.prevent="$refs.darkPhoto.click()"
                                class="flex items-center">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                                </svg>
                                {{ __('modules.settings.uploadDarkModeLogo') }}
                            </x-secondary-button>

                            @if (global_setting()->dark_logo)
                                <x-secondary-button type="button" wire:click="deleteDarkLogo"
                                    class="flex items-center">
                                    ...
                                    {{ __('modules.settings.removeDarkModeLogo') }}
                                </x-secondary-button>
                            @endif
                        </div>

                        <x-input-error for="darkPhoto" class="mt-2" />

                        <p class="mt-1 text-xs text-gray-400">
                            @lang('modules.settings.darkModeLogoUploadHelp')
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        <x-checkbox name="pwaAlertShow" id="pwaAlertShow" wire:model.defer="pwaAlertShow" />
                        <div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        @endforeach
            ...
            <div class="flex items-center justify-end space-x-3">
                <x-button wire:loading.attr="disabled" wire:target="photo, darkPhoto">
                    @lang('app.save')
                </x-button>
            </div>
        </div>
